...
class MetaRLAlgorithm(metaclass=abc.ABCMeta):
    ...
    def _do_eval(self, indices, epoch, wideeval=False):
        ...
        for idx in indices:
            ...
            for r in range(self.num_evals):
                paths, test_suc = self.collect_paths(idx, epoch, r, wideeval=wideeval)
            ...
            suc_rate.append(test_suc)
        n = min([len(t) for t in online_returns])
        online_returns = [t[:n] for t in online_returns]
        return final_returns, online_returns, suc_rate
    ...
    def evaluate(self, epoch):
        ...
        train_final_returns, train_online_returns, train_suc_rate = self._do_eval(indices, epoch)
        eval_util.dprint('train online returns')
        eval_util.dprint(train_online_returns)

        ### test tasks
        eval_util.dprint('evaluating on {} test tasks'.format(len(self.eval_tasks)))
        test_final_returns, test_online_returns, test_suc_rate = self._do_eval(self.eval_tasks, epoch, wideeval=True)
        ...
        avg_train_suc = np.mean(train_suc_rate)
        avg_test_suc = np.mean(test_suc_rate)
        ...
        self.eval_statistics['Averagetrainsuc_rate'] = avg_train_suc
        self.eval_statistics['Averagetestsuc_rate'] = avg_test_suc
